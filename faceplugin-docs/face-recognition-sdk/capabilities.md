---
description: >-
  Faceplugin Face Recognition capabilities. On-premise offline face matching, 1:1 and mobile 1:N
  Identify, NIST FRVT evaluated, Face Recognition API on port 8083, optional PAD.
---
...
`templateExtraction` / `POST /api/feature` returns an embedding you persist in **your** database. Faceplugin does not host a cloud person gallery for commercial Apps.

## Passive 2D liveness on Identify

Mobile **Identify** includes **passive 2D liveness** (default threshold **0.5**). That is matching-oriented PAD on the enrollment/identify path.

For standalone iBeta Level 2 **class** PAD without enrollment, use the [Face Liveness Detection SDK](../liveness-detection-sdk/capabilities.md).
...
Recognition-only images: `faceplugin/face-recognition`. Combined Recognition + Liveness (same port, adds `POST /api/liveness`): [+ Liveness Linux](face-recognition-sdk-linux.md) / [Windows](face-recognition-sdk-windows.md).
...
* [Face Recognition SDK](README.md) · [Try it](../resources/try-it.md)
