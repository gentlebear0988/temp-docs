...
Face Recognition as an HTTP API (or .NET bindings) on **your** machine. Still-image detect, quality, template extract, 1:1 match, and template similarity. Combined Recognition + Liveness Apps also expose `POST /api/liveness` on the same port. There is **no** server-side 1:N gallery (`POST /api/identify` does not exist).

Typical call order: `GET /api/machinecode` → send `FPMC1.…` → `POST /api/activate` → `POST /api/detect` / `match` / `similarity` (and `/liveness` on the combined App). Default port **8083**.
...
**Recognition + liveness in one App?** Yes — [+ Liveness Linux](face-recognition-sdk-linux.md) / [Windows](face-recognition-sdk-windows.md). Or keep Face Liveness on **8084** as a separate product.
